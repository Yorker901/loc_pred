...
if st.sidebar.button('Predict'):
    ...
    for prediction in predictions:
        tooltip = f"User ID: {prediction['user_id']}, Location Name: {prediction['location_name']}, Location Point: ({prediction['location_point'][1]}, {prediction['location_point'][0]}), Timestamp: {prediction['timestamp']}"
        folium.Marker(
